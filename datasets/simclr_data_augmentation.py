...
COLOR_JITTER_STRENGTH = 1.0     # default value from official code
...
def random_apply(func, p, x):
    ...
    return tf.cond(
        tf.less(tf.random_uniform([], minval=0, maxval=1, dtype=tf.float32), tf.cast(p, tf.float32)),
        lambda: func(x),
        lambda: x)
...
def _compute_crop_shape(image_height, image_width, aspect_ratio, crop_proportion):
    ...
    def _requested_aspect_ratio_wider_than_image():
        crop_height = tf.cast(tf.rint(crop_proportion / aspect_ratio * image_width_float), tf.int32)
    ...
    def _image_wider_than_requested_aspect_ratio():
        ...
        crop_width = tf.cast(tf.rint(crop_proportion * aspect_ratio * image_height_float), tf.int32)
        return crop_height, crop_width
...
def random_color_jitter(image, color_jitter_strength, p=1.0):
    def _transform(image):
        # color_jitter_t = functools.partial(color_jitter, strength=FLAGS.color_jitter_strength)
        color_jitter_t = functools.partial(color_jitter, strength=color_jitter_strength)
